...
const config = {
  cache: { // Note: html, js, and css caching is handled automatically
    ...
    include: ['/manifest.json'], // paths to static files that you want precached. to update cache for a particular asset, add a version number with ?v=, the actual file name can remain the same
...
const dev = ['localhost', '127.0.0.1', '2af5a9f2a61f.ngrok-free.app'].includes(self.location.hostname);
if (dev) {
